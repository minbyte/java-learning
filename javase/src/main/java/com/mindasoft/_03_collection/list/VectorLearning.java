 ...
 *
 *  Vector没有丝线Serializable接口，因此它不支持序列化，
 *  实现了Cloneable接口，能被克隆，
 ...
 * 2、注意扩充容量的方法ensureCapacityHelper。与ArrayList相同，
 *    Vector在每次增加元素（可能是1个，也可能是一组）时，都要调用该方法来确保足够的容量。
 *    当容量不足以容纳当前的元素个数时，就先看构造方法中传入的容量增长量参数CapacityIncrement是否为0，如果不为0，
 *    就设置新的容量为就容量加上容量增长量，如果为0，就设置新的容量为旧的容量的2倍，
